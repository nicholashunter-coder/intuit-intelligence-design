...
We attempt to resolve favicons via the page itself first, then fall back to Google's favicon API (`google.com/s2/favicons`). Some sites block direct page fetches (403/404), which means we can't extract a native favicon. In those cases the Google API fallback usually works, but quality and freshness vary. See [Source Metadata](../research/source-metadata.md) for the full resolution strategy.

### What's the difference between the inline treatment options?

We explored five levels of visual weight — from a simple superscript number up to a full chip with favicon and domain. The tradeoff is attribution visibility vs. reading flow disruption. See [Inline Source Treatments](../design/explorations/inline-treatments.md) for the full comparison.
...
See the token table in [Card Interactions](../prototypes/card-interactions.md#design-tokens-used). Formal design tokens are on the roadmap.
